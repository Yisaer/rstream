...
impl Serialize for Datum {
        ...
        S: Serializer,
    {
        match self {
            ...
            Datum::Float(f) => serializer.serialize_f64(*f),
...
impl<'de> Deserialize<'de> for Datum {
        ...
        D: Deserializer<'de>,
    {
        ...
        match value {
            serde_json::Value::Number(n) if n.is_i64() => Ok(Datum::Int(n.as_i64().unwrap())),
            serde_json::Value::Number(n) if n.is_f64() => Ok(Datum::Float(n.as_f64().unwrap())),
            serde_json::Value::String(s) => Ok(Datum::String(s)),
            serde_json::Value::Bool(b) => Ok(Datum::Boolean(b)),
